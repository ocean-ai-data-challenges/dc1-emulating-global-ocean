...
class DC1Evaluation:
        ...
        for alias in aliases:
            if alias == "glorys_cmems":
                transforms_dict["glorys_cmems"] = dataset_manager.get_transform(
                    ...
                    interp_ranges=TARGET_DIM_RANGES_SURFACE,
                    weights_path=self.args.regridder_weights,
                    depth_coord_vals=TARGET_DEPTH_VALS_SURFACE,
                )
            #if alias == "glonet":
            #    transforms_dict["glonet"] = dataset_manager.get_transform(
            #    "standardize_to_surface",
    ...
    def setup_dataset_manager(self, list_all_references: list[str]) -> None:
        ...
        manager = MultiSourceDatasetManager(
            ...
            target_dimensions=TARGET_DIM_RANGES_SURFACE,
    ...
    def run_eval(self) -> None:
        ...
        for alias in self.dataset_references.keys():
            dataset_json_path = os.path.join(self.args.result_dir, f"results_{alias}.json")
            logger.info(f"Preparing evaluation for dataset '{alias}'. Results will be saved to {dataset_json_path}")
        ...
        logger.info(f"Evaluation finished. Results saved to {self.args.data_directory}")