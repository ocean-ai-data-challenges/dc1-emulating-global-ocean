...
::
    ...
    python -m dc1.submit validate /path/to/my_model.zarr --model-name MyModel

    # Full submission (validate  evaluate  leaderboard):
    python -m dc1.submit run /path/to/my_model.zarr \\
        --model-name MyModel \\
    ...
    python -m dc1.submit run /path/to/my_model.zarr --model-name MyModel --force
"""
...
# xarray scans optional backend entrypoints and may emit a known non-fatal
# warning for argopy's "argo" engine in some environments/workers.
_ARGO_BACKEND_WARNING_PATTERN = r"Engine 'argo' loading failed:.*"
warnings.filterwarnings(
    "ignore",
    message=_ARGO_BACKEND_WARNING_PATTERN,
    category=RuntimeWarning,
    module=r"xarray\.backends\.plugins",
)

_argo_warn_filter = "ignore:Engine 'argo' loading failed:RuntimeWarning:xarray.backends.plugins"
_existing_pythonwarnings = os.environ.get("PYTHONWARNINGS", "")
if _argo_warn_filter not in _existing_pythonwarnings:
    os.environ["PYTHONWARNINGS"] = (
        f"{_existing_pythonwarnings},{_argo_warn_filter}"
        if _existing_pythonwarnings
        else _argo_warn_filter
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  # Quick validation:\n"
            "  python -m dc1.submit validate /data/my_model.zarr --model-name MyModel\n\n"
            "  # Full submission:\n"
            "  python -m dc1.submit run /data/my_model.zarr \\\n"
    ...
    run_parser.add_argument(
        "--skip-map-data-pack",
        action="store_true",
        help="Do not auto-generate docs/source/_extra/leaderboard/map_data.tar.gz after a successful run.",
    )
    ...
    info_parser.add_argument(
        ...
        type=str,
        default="dc1",
        help="Data Challenge config name (default: dc1).",
    )
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if exit_code == 0 and not args.skip_map_data_pack:
...
def _pack_leaderboard_map_data_archive() -> None:
    ...
    if not map_data_dir.is_dir():
        print(
            "[submit] map_data directory not found; skipped leaderboard archive generation "
            f"({map_data_dir})."
        )
        return

    print("[submit] Packing leaderboard map data archive ...")
    result = subprocess.run([sys.executable, str(pack_script)], check=False)
    if result.returncode != 0:
        print(
            "[submit] WARNING: map_data archive generation failed. "
